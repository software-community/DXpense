...
describe("Dxpense", function () {
  ...
  it("should add a new trip and owner as person", async function () {
    ...
    const trip = await dxpense.connect(alice).getTrip(1); // 0 is the test trip from constructor
  ...
  it("should add a person to a trip", async function () {
    ...
    const trip = await dxpense.getTrip(1);
  ...
  it("should not add the same person twice to a trip", async function () {
    ...
    await dxpense.AddPerson(1, bob.address, "Bob");
    await expect(
      dxpense.AddPerson(1, bob.address, "Bob")
  ...
  it("should revert if expender not in trip", async function () {
    await dxpense.AddTrip("Goa Trip", "Alice");
    await expect(
      dxpense.AddExpense(1, 1000, "Dinner", bob.address, [owner.address])
    ).to.be.revertedWith("Expender not in trip");
  });

  it("should revert if participant not in trip", async function () {
    await dxpense.AddTrip("Goa Trip", "Alice");
    await expect(
      dxpense.AddExpense(1, 1000, "Dinner", owner.address, [bob.address])
    ).to.be.revertedWith("Participant not in trip");
  });

  it("should calculate debts correctly", async function () {
    await dxpense.connect(alice).AddTrip("Goa Trip", "Alice");
    await dxpense.connect(alice).AddPerson(1, bob.address, "Bob");
    ...
    await dxpense.connect(alice).calculateDebts(1);
